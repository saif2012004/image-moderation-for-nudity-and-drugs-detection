...
from .weapons_detector import detect_weapons

# Set up logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Moderation categories - UPDATED: Only nudity and drugs
...
class ModerationService:
    ...
    def __init__(self):
        ...
        logging.info("🚀 Moderation service initialized with nudity and drugs detection only")
    ...
    async def _detect_weapons_real(self, image_data: bytes, filename: str) -> float:
        """
        Real weapons detection using AI
        """
        try:
            # Use AI weapons detection
            result = detect_weapons(image_data, detailed=True)
            
            confidence = result.get('confidence', 0.0)
            is_weapons = result.get('is_weapons', False)
            method = result.get('method', 'unknown')
            
            logger.info(f"Weapons detection for {filename}: method={method}, weapons={is_weapons}, confidence={confidence:.3f}")
            
            return confidence
            
        except Exception as e:
            logger.error(f"Error in weapons detection for {filename}: {e}")
            return 0.05  # Conservative fallback
    ...
    async def _detect_category_conservative(self, image: Image.Image, category: str, filename: str) -> float:
        ...
        try:
            ...
            if category == "drugs":
                ...
                return await self._detect_weapons_conservative(image_cv, filename)
            elif category == "hate_symbols":
                return await self._detect_hate_symbols_conservative(image_cv, filename)
            else:
                return 0.05
    ...
    async def _detect_weapons_conservative(self, img_bgr: np.ndarray, filename: str) -> float:
        """Conservative weapon detection"""
        try:
            # Look for specific weapon indicators
            metallic_confidence = self._detect_realistic_metal(img_bgr)
            shape_confidence = self._detect_realistic_weapon_shapes(img_bgr)
            edge_confidence = self._detect_realistic_sharp_edges(img_bgr)
            
            # Require multiple indicators for high confidence
            if metallic_confidence > 0.6 and shape_confidence > 0.6:
                weapon_confidence = (metallic_confidence + shape_confidence + edge_confidence) / 3
            elif metallic_confidence > 0.7 or shape_confidence > 0.7:
                weapon_confidence = max(metallic_confidence, shape_confidence) * 0.7
            else:
                weapon_confidence = max(metallic_confidence, shape_confidence, edge_confidence) * 0.4
            
            logger.info(f"Weapon detection confidence: {weapon_confidence:.2f}")
            return weapon_confidence
            
        except Exception as e:
            logger.error(f"Error in weapon detection for {filename}: {e}")
            return 0.05
    
    async def _detect_hate_symbols_conservative(self, img_bgr: np.ndarray, filename: str) -> float:
        """Conservative hate symbol detection"""
        try:
            img_gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
            
            # Look for specific symbol patterns
            symbol_confidence = self._detect_realistic_symbols(img_gray)
            text_confidence = self._detect_realistic_hate_text(img_gray)
            geometric_confidence = self._detect_realistic_geometric_patterns(img_gray)
            
            # Require strong evidence
            hate_confidence = max(symbol_confidence, text_confidence, geometric_confidence)
            
            # Only flag if very confident
            if hate_confidence < 0.6:
                hate_confidence *= 0.5  # Reduce confidence if not very sure
            
            logger.info(f"Hate symbol detection confidence: {hate_confidence:.2f}")
            return hate_confidence
            
        except Exception as e:
            logger.error(f"Error in hate symbol detection for {filename}: {e}")
            return 0.05
    ...
    def _detect_realistic_metal(self, img_bgr: np.ndarray) -> float:
        """Detect realistic metallic surfaces"""
        try:
            # Very conservative metallic detection
            return 0.05  # Placeholder
            
        except Exception:
            return 0.05
    
    def _detect_realistic_weapon_shapes(self, img_bgr: np.ndarray) -> float:
        """Detect realistic weapon shapes"""
        try:
            # Very conservative weapon shape detection
            return 0.05  # Placeholder
            
        except Exception:
            return 0.05
    
    def _detect_realistic_sharp_edges(self, img_bgr: np.ndarray) -> float:
        """Detect realistic sharp edges"""
        try:
    ...
    def _detect_realistic_symbols(self, img_gray: np.ndarray) -> float:
        """Detect realistic hate symbols"""
        try:
            # Very conservative symbol detection
            return 0.05  # Placeholder
            
        except Exception:
            return 0.05
    
    def _detect_realistic_hate_text(self, img_gray: np.ndarray) -> float:
        """Detect realistic hate text"""
        try:
    ...
    def _detect_realistic_geometric_patterns(self, img_gray: np.ndarray) -> float:
        """Detect realistic geometric hate patterns"""
        try:
